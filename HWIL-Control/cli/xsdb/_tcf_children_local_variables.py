...
#################################################################################################
class TcfChildrenLocalVariables(TcfChildren):
    def __init__(self, node: TcfNodeStackFrame):
        super().__init__(node)
        self._node = node
    ...
    def startDataRetrieval(self):
                ...
                if exps_children._command is token and error is None:
                    ...
                    for id in contexts:
                        if id in exps_children.node.info.get_nodes().keys():
                            ...
                            n = TcfNodeExpression(exps_children.node, id, None)
        ...
        stack_trace_cache = self._node.parent.get_stack_trace().get_stack_children()
        if not stack_trace_cache.validate(self):
            return False  # node.getFrameNo() is not valid
        # if self._node.getFrameNo() < 0:
        #     set(None, None, {})
        #     return True
