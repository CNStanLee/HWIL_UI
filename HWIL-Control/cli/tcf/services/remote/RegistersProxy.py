...
class RegistersProxy(registers.RegistersService):
    ...
    def parseIpxact(self, context_id, buf, done):
        done = self._makeCallback(done)
        service = self

        class ParseIpExactCommand(Command):
            def __init__(self):
                super(ParseIpExactCommand, self).__init__(service.channel,
                                                          service,
                                                          "parseIPXACT",
                                                          (context_id, buf,))
            ...
            def done(self, error, args):
                if not error:
                    assert len(args) == 1
                    error = self.toError(args[0])
                done.doneparseIpxact(self.token, error)

        return ParseIpExactCommand().token
